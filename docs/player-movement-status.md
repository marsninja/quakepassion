...
Run `./qp` and press **F4** to enable walking. **WASD** moves, **Space** jumps,
and **F4** returns to flight. `QP_GAME=q2 QP_WALK=1 ./qp` starts walking in Q2;
use `q1` or `q3` for the other games. Changing levels resets to flight.
The development arena remains fly-only.

## Compiler requirement

The current source requires the compiler fixes in
[Jac PR #9344](https://github.com/jaseci-labs/jac/pull/9344).
...
```sh
export JAC_DEV_SOURCE=/Users/marsninja/repos/jaseci-wt/qp-relative-types/jac
...
The 60-test unit suite passes with the patched compiler, including player-size
...
checks, not complete map traversal or original-game movement parity.
...
- Q3 curved patches render, but only brushes collide; patch surfaces without
  supporting brushes can be passed through.
- Doors, lifts, trains and buttons remain static. Brush entities use translation
  only; rotation and animated transforms are not implemented.
- Triggers, jump pads, teleporters, liquids, swimming and crouching need behavior.
